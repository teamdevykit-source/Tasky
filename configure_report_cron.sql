-- Complete setup for scheduled report email delivery.
-- Run this once in the Supabase SQL editor for the active project.
--
-- Requires the same vault secrets from configure_task_reminder_cron.sql:
--   tasky_project_url
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Missing vault secret: tasky_project_url. Run configure_task_reminder_cron.sql first.';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM vault.decrypted_secrets WHERE name = 'tasky_publishable_key'
  ) THEN
    RAISE EXCEPTION 'Missing vault secret: tasky_publishable_key. Run configure_task_reminder_cron.sql first.';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM vault.decrypted_secrets WHERE name = 'tasky_reminder_cron_secret'
  ) THEN
    RAISE EXCEPTION 'Missing vault secret: tasky_reminder_cron_secret. Run configure_task_reminder_cron.sql first.';
  END IF;
END;
$$;
...
FROM cron.job
WHERE jobname = 'tasky-send-report-emails';

SELECT cron.schedule(
  'tasky-send-report-emails',
  '* * * * *',
  $$
    SELECT net.http_post(
      ...
      body := '{"process_due_schedules":true}'::jsonb
    ) AS request_id;
  $$
);
